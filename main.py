...
@bot.event
async def on_ready():
    # Itt a guild ID-t lecseréltem globális szinkronizálásra, 
    # hogy minden szerveren működjön, ahol bent van a bot.
    await bot.tree.sync()
...
@bot.tree.command(name="videó", description="Videós bizonyíték feltöltése")
@app_commands.check(mod_check)
async def video(i: discord.Interaction, szoveg: str, video: discord.Attachment):
    # Railway-en fontos a defer, mert a feltöltés időbe telhet
