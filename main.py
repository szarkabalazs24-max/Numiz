...
async def on_message(msg):
    ...
    # MODERÁTOROKNAK SZABAD LINKET KÜLDENI
    ...
    # LINK SZŰRÉS (Csak ha nem moderátor)
    ...
    # KÁROMKODÁS (Mindenkire vonatkozik)
...
@bot.tree.command(name="video", description="Videós bizonyíték küldése")
@app_commands.check(mod_check)
async def video_send(i: discord.Interaction, tag: discord.Member, szoveg: str, link: str):
    data = load_json(VIDEO_COUNTER_FILE)
    current_count = data.get("count", 147)
    
    # A képen látható formátum
    valasz = (
        f"{tag.mention} {szoveg}\n"
        f"📸 **{current_count}. Bizonyíték:**\n"
        f"{link}"
    )
    
    await i.response.send_message(valasz)
    
    # Számláló növelése
    data["count"] = current_count + 1
    save_json(VIDEO_COUNTER_FILE, data)

# ================= TOVÁBBI PARANCSOK (Változatlan) =================
