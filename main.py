...
# Anti-spam tároló a háttérben futó automatikához
user_messages = {}
...
async def on_message(msg):
    ...
    # --- AUTOMATIKUS ANTI-SPAM (HÁTTÉRBEN) ---
    if not is_mod:
...
async def welcome_set(i: discord.Interaction, csatorna: discord.TextChannel):
    save_json(WELCOME_FILE, {"channel_id": csatorna.id}); await i.response.send_message("✅ Beállítva", ephemeral=True)

@bot.tree.command(name="kilépő_beállítás")
@app_commands.check(mod_check)
async def leave_set(i: discord.Interaction, csatorna: discord.TextChannel):
    save_json(LEAVE_FILE, {"channel_id": csatorna.id}); await i.response.send_message("✅ Beállítva", ephemeral=True)

@bot.tree.command(name="autorole_beállítás")
@app_commands.check(mod_check)
async def autorole_set(i: discord.Interaction, rang: discord.Role):
    save_json(AUTO_ROLE_FILE, {"role_id": rang.id}); await i.response.send_message("✅ Beállítva", ephemeral=True)
...
async def warn_del(i: discord.Interaction, tag: discord.Member, szám: int):
    ...
    if 0 < szám <= len(warns): warns.pop(szám-1); save_json(WARN_FILE, data); await i.response.send_message("✅ Törölve")
...
async def mute(i: discord.Interaction, tag: discord.Member, perc: int, indok: str):
    ...
    await i.response.send_message(embed=make_embed("🔇 Mute", f"{tag.mention} - {perc}m\nIndok: {indok}", discord.Color.red()))

@bot.tree.command(name="némítás_feloldás")
@app_commands.check(mod_check)
async def unmute(i: discord.Interaction, tag: discord.Member):
    await tag.timeout(None); await i.response.send_message("🔊 Feloldva")

# ================= KIRÚGÁS HELYÉRE AZ ANTI-SPAM EMBED =================

@bot.tree.command(name="kirúgás")
@app_commands.check(mod_check)
async def kick(i: discord.Interaction, tag: discord.Member, indok: str):
    # A kirúgás funkció helyett mostantól manuális Anti-Spam figyelmeztetést küld
    ...
    embed = make_embed(
        ...
        f"**Indok:** {indok}\n"
        f"**Büntetés:** 15 perc némítás\n"
        f"**Moderátor:** {i.user.mention}",
        discord.Color.blue()
    )
...
async def ban(i: discord.Interaction, tag: discord.Member, indok: str):
    ...
    await i.response.send_message(embed=make_embed("🚫 Ban", f"{tag.mention}\n📄 {indok}", discord.Color.dark_red()))
