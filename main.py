...
async def on_message(msg):
    ...
    if not is_mod and re.search(LINK_REGEX, txt):
        ...
        await msg.channel.send(embed=make_embed("🔗 Link szűrő", f"👤 {msg.author.mention}\n📄 Indok: Tiltott link\n🔇 Némítás: {mute_time} perc", discord.Color.red()))
        return

    # --- AUTO KÁROMKODÁS SZŰRŐ ---
    if not is_mod and any(w in txt for w in FORBIDDEN_WORDS):
        await msg.delete()
        data = load_json(WARN_FILE); data.setdefault(uid, []); data[uid].append("Káromkodás"); save_json(WARN_FILE, data)
        mute_time = len(data[uid]) * 2
        await msg.author.timeout(datetime.timedelta(minutes=mute_time))
        await msg.channel.send(embed=make_embed("🤬 Szűrő", f"👤 {msg.author.mention}\n📄 Indok: Káromkodás\n🔇 Némítás: {mute_time} perc", discord.Color.orange()))
        return
