...
async def on_message(msg):
    ...
    # Moderátorok/Adminok küldhetnek linket
...
async def video_send(i: discord.Interaction, szoveg: str, video: discord.Attachment):
    # Ellenőrzés: videó vagy kép fájl-e
    if not any(video.content_type.startswith(t) for t in):
        return await i.response.send_message("❌ Kérlek, videó fájlt vagy képet tölts fel!", ephemeral=True)

    data = load_json(VIDEO_COUNTER_FILE)
    current_count = data.get("count", 147)
    
    # Kért formátum: Szöveg, alatta a sorszám és a fájl
    valasz_szoveg = (
        f"{szoveg}\n"
        f"📸 **{current_count}. Bizonyíték:**"
    )
    
    # Üzenet küldése a feltöltött videóval
    await i.response.send_message(content=valasz_szoveg, file=await video.to_file())
...
async def warn_del(i: discord.Interaction, tag: discord.Member, szám: int):
    ...
    if szám < 1 or szám > len(warns): return await i.response.send_message("❌ Hibás sorszám", ephemeral=True)
    warns.pop(szám - 1)
    save_json(WARN_FILE, data)
    await i.response.send_message(embed=make_embed("🧹 Figyelmeztetés törölve", f"👤 {tag.mention}\n📉 Maradt: {len(warns)}", discord.Color.green()))
...
async def idban(i: discord.Interaction, uid: str, indok: str):
    ...
    await i.response.send_message(embed=make_embed("🚫 ID kitiltás", f"🆔 {uid}\n📄 {indok}", discord.Color.dark_red()))

@bot.tree.command(name="id_unban")
@app_commands.check(mod_check)
async def id_unban(i: discord.Interaction, uid: str):
    user = await bot.fetch_user(int(uid))
    await i.guild.unban(user)
    await i.response.send_message(embed=make_embed("🔓 Kitiltás feloldva", f"🆔 {uid}", discord.Color.green()))
